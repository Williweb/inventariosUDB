...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6">
        <form method="POST" action="/productos/{{ $producto->id }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Código de barra</label>
                <input type="number" name="barra" value="{{ old('barra', $producto->barra) }}" required
            ...
            <div>
                ...
                <input type="text" name="descripcion" value="{{ old('descripcion', $producto->descripcion) }}" required maxlength="255"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition"
            ...
            <div>
                ...
                <input type="number" name="precio" value="{{ old('precio', $producto->precio) }}" required min="0" step="0.01"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition"
                       placeholder="0.00">
            </div>
            ...
            <div>
                ...
                <input type="url" name="imagen" value="{{ old('imagen', $producto->imagen) }}" maxlength="500"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition"
                       placeholder="https://...">
            </div>
