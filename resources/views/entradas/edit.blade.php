...
@section('title', 'Editar entrada — Inventarios UDB')
@section('page-title', 'Editar entrada')
@section('page-subtitle', '{{ $entrada->producto->descripcion }}')
...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6">
        <form method="POST" action="/entradas/{{ $entrada->id }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="id_producto" required
                    ...
                    @foreach($productos as $producto)
                        <option value="{{ $producto->id }}" {{ old('id_producto', $entrada->id_producto) == $producto->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="cantidad" value="{{ old('cantidad', $entrada->cantidad) }}" required min="1"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Precio unitario</label>
                <input type="number" name="precio_unitario" value="{{ old('precio_unitario', $entrada->precio_unitario) }}" required min="0" step="0.01"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Proveedor</label>
                <input type="text" name="proveedor" value="{{ old('proveedor', $entrada->proveedor) }}" required maxlength="150"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition">
            </div>
            ...
            <div>
                ...
                <input type="text" name="descripcion" value="{{ old('descripcion', $entrada->descripcion) }}" maxlength="255"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Fecha</label>
                <input type="date" name="fecha" value="{{ old('fecha', $entrada->fecha->format('Y-m-d')) }}" required
                       class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition">
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="/entradas" class="text-sm text-gray-500 hover:text-gray-700 transition">Cancelar</a>
            </div>
